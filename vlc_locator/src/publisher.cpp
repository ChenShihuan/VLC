...
private:
	...
	image_transport::Subscriber second_image_sub_; //定义第二个ROS图象接收器
	image_transport::Subscriber third_image_sub_; //定义第三个ROS图象接收器
	image_transport::Publisher image_pub_;
	image_transport::Publisher second_image_pub_;
	image_transport::Publisher third_image_pub_;
	ros::Publisher msgPointPub;
	ros::Publisher second_msgPointPub;
	ros::Publisher third_msgPointPub;

public:
	IMAGE_LISTENER_and_LOCATOR()
		:it_(nh_) //构造函数
	{
		image_sub_ = it_.subscribe("/tb3_0/mvcam/image", 1, boost::bind(&IMAGE_LISTENER_and_LOCATOR::convert_callback, this, _1, "tb3_0")); //定义图象接受器，订阅话题是“camera/image”
		second_image_sub_ = it_.subscribe("/tb3_1/mvcam/image", 1, boost::bind(&IMAGE_LISTENER_and_LOCATOR::convert_callback, this, _1, "tb3_1")); //定义图象接受器，订阅话题是“camera/image”
		third_image_sub_ = it_.subscribe("/tb3_2/mvcam/image", 1, boost::bind(&IMAGE_LISTENER_and_LOCATOR::convert_callback, this, _1, "tb3_2")); //定义图象接受器，订阅话题是“camera/image”
		
		image_pub_ = it_.advertise("/tb3_0/location/image_show", 1); //定义ROS图象发布器
		second_image_pub_ = it_.advertise("/tb3_1/location/image_show", 1); //定义ROS图象发布器
		third_image_pub_ = it_.advertise("/tb3_2/location/image_show", 1); //定义ROS图象发布器
		
		msgPointPub = nh_.advertise<geometry_msgs::PointStamped>("tb3_0/location", 1000);
		second_msgPointPub = nh_.advertise<geometry_msgs::PointStamped>("tb3_1/location", 1000);
		third_msgPointPub = nh_.advertise<geometry_msgs::PointStamped>("tb3_2/location", 1000);
	...
	//-----------------------------------------------------------------------------------------------
	void convert_callback(const sensor_msgs::ImageConstPtr& msg, std::string server)
	{
		...
		try
		{
			...
			else if (server == "tb3_1")
			{
				second_image_pub_.publish(msg);
				a = 1;
			}
			else if (server == "tb3_2")
			{
				third_image_pub_.publish(msg);
				a = 2;
			}
		}
		...
		image_process(cv_ptr->image, a); //得到了cv::Mat类型的图象，在CvImage指针的image中，将结果传送给处理函数
	}

	//----------------------------------【图象处理主函数】----------------------------------------------
	//      描述：这是图象处理主函数，一般会把图像处理的主要程序写在这个函数中。
	//-----------------------------------------------------------------------------------------------
	void image_process(cv::Mat img, int a)
	{
		...
		while (ros::ok())
		{
			...
			msgPointStamped.point.x = msgPointStamped.point.x * 100;
			msgPointStamped.point.y = msgPointStamped.point.y * 100;
			msgPointStamped.point.z = msgPointStamped.point.z * 100;
			...
			{
				ss << '\n' << "tb3_0/VLC"
					<< '\n' << "x = " << msgPointStamped.point.x
					<< '\n' << "y = " << msgPointStamped.point.y << count;
				//<< '\n'<< msgPointStamped.point.z 

				msgPointStamped.header.stamp = ros::Time::now();
				msgPointStamped.header.frame_id = "tb3_0/map";
			...
			else if (a = 1)
			{
				...
				msgPointStamped.header.stamp = ros::Time::now();
				msgPointStamped.header.frame_id = "tb3_1/map";
				second_msgPointPub.publish(msgPointStamped);
			...
			else if (a = 2)
			{
				...
				msgPointStamped.header.stamp = ros::Time::now();
				msgPointStamped.header.frame_id = "tb3_2/map";
				third_msgPointPub.publish(msgPointStamped);
