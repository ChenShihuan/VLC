...
//OpenCV中threshold自动阈值，类似matlab中的graythresh。为了二值化
...
//-----------------------------------【Get_coordinate()函数】------------------------------------
//      描述：双灯定位，灰度图像传入cv::Mat img
//-----------------------------------------------------------------------------------------------  

struct XYZ Get_coordinate()
{

	// 图像读取及判断
//	cv::Mat grayImage = img;
	//cv::Mat image = cv::imread("/home/topeet/test.jpg", CV_LOAD_IMAGE_COLOR);
	cv::Mat grayImage = cv::imread("20--20-逆45.bmp",0);
	...
	double Center_X = 417.5;
	double Center_Y = 341.5;
	//三个LED灯具的真实位置
	double x1 = -330;
	double y1 = -330;
	double x2 = 330;
